...
class HexGridView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private val holdPressHandler = android.os.Handler(android.os.Looper.getMainLooper())
    private var holdPressRunnable: Runnable? = null
    private var isHoldPress = false

    val cornerColors = mutableListOf(Color.RED, Color.GREEN, Color.BLUE)
    ...
    private fun updateCornerColor(index: Int, color: Int) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        hexGrid.flatten().forEach { hex ->
            drawHexagon(canvas, hex.x + xOffset, hex.y + yOffset, hex.color, hex.isCorner)
        }
    }

    private fun drawHexagon(canvas: Canvas, x: Float, y: Float, color: Int, isCorner: Boolean) {
        hexPath.reset()
        ...
        if (isCorner) {
            ...
            paint.strokeWidth = 7f
            paint.pathEffect = DashPathEffect(floatArrayOf(7f, 7f), 7f)
            canvas.drawPath(hexPath, paint)
            ...
            if (holdPressedHex?.isCorner == true) {
                ...
                (context as? AppCompatActivity)?.let { activity ->
                    showColorPickerDialog(activity) { selectedColor ->
                        updateCornerColor(cornerIndex, selectedColor)
                        recalculateHexColors()
                    }
                }
            }
    ...
    private fun subtractColors(color1: Int, color2: Int, color3: Int): Int {
        val red = (Color.red(color1) * Color.red(color2) *  Color.red(color3) / 65025).coerceIn(0, 255)
        val green = (Color.green(color1) * Color.green(color2) * Color.green(color3) / 65025).coerceIn(0, 255)
        val blue = (Color.blue(color1) * Color.blue(color2) * Color.blue(color3) / 65025).coerceIn(0, 255)
